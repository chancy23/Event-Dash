$(document).ready(function() {
  ...
  //varialbes to call the database and to call the database root plus the "event" child path
  ...
  var allImages = [
    "./assets/images/colorSplashBG.jpg", 
    "./assets/images/stagelights.jpg", 
    "./assets/images/pinkfireworks.jpg"
  ];
  ...
  //Runs Smarty Streets API to check that entered address is valid.
    function verifyAddress(address) {
      ...
      var state = address[2];
      var zip = address[3];
      var url = "https://us-street.api.smartystreets.com/street-address?auth-id=8909590200091130&candidates=1";
  ...
  //main processes for creating and dispklaying events
  //this is the onclick for when the "Submit" button is pushed from the Add Event form 
  //add input to the DB and create and displays the item on the page
  ...
  //on child event for database to be call to get the snapshot values are added to show in the thumbnail
  eventRef.on("child_added", function(snapshotChild) {
    ...
    //I think this may be in the wrong spot, if we are not adding it to the event thumbnail it should be moved
    // lat = cv.lat;
    // lng = cv.lng;
    // console.log(lat);
    // console.log(lng);
  ...
  $("#allEvents").on("click", ".goToDetails", function(event) {
    ...
    query.on("value", function(snapshot) {
      ...
      $("#detailZip").text(zip);
      //add the map to the Event Details modal
      map(latLong);
    });
  });
